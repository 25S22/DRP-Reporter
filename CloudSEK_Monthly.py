...
CONFIGURATION — only 7 things to set:
...
COL_EVENT_COUNT   : exact column name holding the per-incident event count
(leave as “” to skip the Total Events Closed KPI)
...
# ↓ NEW — set to the column that holds the per-incident event count.

# Leave as “” to skip the “Total Events Closed” insight box.

COL_EVENT_COUNT   = “Event Count”
...
# STEP 3g — TOTAL EVENT COUNT (NEW)

# —————————————————————————

def compute_total_event_count(filtered_closure_raw, sheet_names):
“””
Sums COL_EVENT_COUNT across all closed-in-range incidents (deduplicated
by COL_INCIDENT_ID). Returns an integer total.
Skipped gracefully if COL_EVENT_COUNT is blank or absent in all sheets.
“””
if not COL_EVENT_COUNT:
return 0
frames = []
for name in sheet_names:
df = filtered_closure_raw.get(name, pd.DataFrame())
if df.empty or COL_EVENT_COUNT not in df.columns:
continue
cols = [c for c in [COL_INCIDENT_ID, COL_EVENT_COUNT] if c in df.columns]
frames.append(df[cols].copy())
if not frames:
print(f”  INFO: Column ‘{COL_EVENT_COUNT}’ not found — Total Events KPI skipped.”)
return 0
combined = pd.concat(frames, ignore_index=True)
if COL_INCIDENT_ID in combined.columns:
combined = combined.drop_duplicates(subset=[COL_INCIDENT_ID])
total = 0
for val in combined[COL_EVENT_COUNT]:
try:
v = float(val)
if not pd.isna(v):
total += int(v)
...
pass
print(f”\n  Total event count (closed incidents in range): {total:,}”)
return total
...
total_events_closed,          # ← NEW parameter
NAVY, WHITE):
“””
Writes a 2-box insight strip starting at `cursor`.
Box 1 (left,  cols 0-6):  Most Impacted Module
Box 2 (right, cols 7-14): Total Events Closed  (hidden when COL_EVENT_COUNT is blank)
...
# Decide column split: use full width for box 1 when event count is absent
show_events = bool(COL_EVENT_COUNT and total_events_closed >= 0)
if show_events:
    B1_C1, B1_C2 = 0, 6     # left box  — cols 0-6
    B2_C1, B2_C2 = 7, 14    # right box — cols 7-14
else:
    B1_C1, B1_C2 = 0, 14    # single full-width box
    B2_C1 = B2_C2 = None

# ── BOX 1 — Most Impacted Module ──────────────────────────────────────────
BLU_H = "#2563EB"
BLU_B = "#E0E7FF"

sw.merge_range(cursor, B1_C1, cursor, B1_C2,
               "◆  MOST IMPACTED MODULE",
               _f(bold=True, font_size=9, font_color=WHITE,
                  bg_color=BLU_H, align="center",
                  border=1, top=2, top_color=BLU_H))

sw.merge_range(cursor + 1, B1_C1, cursor + 1, B1_C2,
               most_mod[:28],
               _f(bold=True, font_size=18, font_color="#111827",
                  bg_color=BLU_B, align="center", border=1))

sw.merge_range(cursor + 2, B1_C1, cursor + 2, B1_C2,
               f"{most_cnt:,} incidents in period",
               _f(font_size=9, italic=True, font_color="#4B5563",
                  bg_color=BLU_B, align="center", border=1))

sw.merge_range(cursor + 3, B1_C1, cursor + 3, B1_C2, "",
               _f(bg_color=BLU_H))

# ── BOX 2 — Total Events Closed (NEW) ────────────────────────────────────
if show_events and B2_C1 is not None:
    EVT_H = "#065F46"    # dark green header
    EVT_B = "#D1FAE5"    # light green body

    sw.merge_range(cursor, B2_C1, cursor, B2_C2,
                   "★  TOTAL EVENTS CLOSED",
                   _f(bold=True, font_size=9, font_color=WHITE,
                      bg_color=EVT_H, align="center",
                      border=1, top=2, top_color=EVT_H))

    sw.merge_range(cursor + 1, B2_C1, cursor + 1, B2_C2,
                   f"{total_events_closed:,}",
                   _f(bold=True, font_size=18, font_color="#111827",
                      bg_color=EVT_B, align="center", border=1))

    sw.merge_range(cursor + 2, B2_C1, cursor + 2, B2_C2,
                   f"sum of '{COL_EVENT_COUNT}' — closed in period",
                   _f(font_size=9, italic=True, font_color="#4B5563",
                      bg_color=EVT_B, align="center", border=1))

    sw.merge_range(cursor + 3, B2_C1, cursor + 3, B2_C2, "",
                   _f(bg_color=EVT_H))
...
start_dt, end_dt,
output_path,
total_events_closed=0,     # ← NEW parameter (default 0 for back-compat)
):
import xlsxwriter
import pandas as pd
from datetime import datetime
...
sw.set_row(KPI_TOP,     16)
sw.set_row(KPI_TOP + 1, 46)
sw.set_row(KPI_TOP + 2, 18)
sw.set_row(KPI_TOP + 3,  4)
sw.set_row(KPI_TOP + 4,  8)
...
cursor = _add_insights_panel(
    ...
    mods_sorted, union_count_map, union_module_status,
    res_rate, user_names, user_counts,
    total_events_closed,           # ← NEW argument
    NAVY, WHITE,
)
...
# ── CHART 1 — Stacked horizontal bar (ENLARGED) ───────────────────────────
#
# Key sizing changes vs previous version:
#   • Width:  680 → 960  (+41 %) — gives every module label full breathing room
#   • Height: max(380, NM*42+130) → max(520, NM*65+200)
#             Taller rows mean numbers are never squashed together
#   • gap:    50 → 30  — even fatter bars; impossible to miss individual segments
#   • Data-label font:  9 → 12 bold — easily readable without zooming
#   • Title font:      12 → 14 — clearly dominant heading
#   • Y-axis (module names): size 11 → 12, bold stays True
#   • X-axis (count axis):   size  9 → 11
#   • Legend font:           10 → 11

if NM > 0:
    bar_h = max(520, NM * 65 + 200)   # ← was max(380, NM * 42 + 130)
    bar_w = 960                        # ← was 680

    stacked = wb.add_chart({"type": "bar", "subtype": "stacked"})
    stacked.series_gap_1 = 30                # ← was 50 — fatter bars
    ...
    stacked.add_series({
        ...
        "data_labels": {
            "value":    True,
            "position": "inside_end",
            "font":     {"bold": True, "size": 12, "color": WHITE,   # ← was 9
                         "name": "Calibri"},
        },
    })
    ...
    stacked.add_series({
        ...
        "data_labels": {
            "value":    True,
            "position": "inside_end",
            "font":     {"bold": True, "size": 12, "color": NAVY,    # ← was 9
                         "name": "Calibri"},
        },
    })
    # Series: Open (red)
    ...
    stacked.set_title({
        ...
        "name_font": {"bold": True, "size": 14, "color": NAVY,       # ← was 12
                      "name": "Calibri"},
    })
    stacked.set_legend({
        "position": "bottom",
        "font":     {"bold": True, "size": 11, "color": SUBTLE,      # ← was 10
                     "name": "Calibri"},
        "border":   {"color": "#E5E7EB"},
        "fill":     {"color": OFFWHITE},
    })
    stacked.set_x_axis({
        "num_font":        {"size": 11, "color": SUBTLE},             # ← was 9
        "major_gridlines": {
            "visible": True,
    ...
    stacked.set_y_axis({
        "num_font":        {"size": 12, "bold": True, "color": NAVY}, # ← was 10
        "line":            {"none": True},
...
# Reserve enough rows for the taller chart
chart_rows = max(NM * 4 + 14, 32)    # ← was max(NM * 3 + 8, 24)
...
unique_emails                                  = extract_emails_closed_resolved(
    processed_raw, sheet_names)
# ↓ NEW — compute total event count for closed incidents in range
total_events_closed                            = compute_total_event_count(
    filtered_closure_raw, sheet_names)
...
build_workbook(
    ...
    start_dt, end_dt,
    output_path,
    total_events_closed=total_events_closed,   # ← NEW
)
